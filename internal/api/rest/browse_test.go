...
func TestDataTracksByLibrary(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`FROM tracks WHERE library_id = ANY($1)`)).
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "library_id", "title", "cover_image_id",
			...
			AddRow("t-001", "lib-001", "Song A", nil, 200, 128000, 44100, 2, "/m/a.mp3", 8, "mp3", "mp3", "", "", "h",
				0, 0, 0, 0, nil, nil, 1, "", now, now).
			AddRow("t-002", "lib-001", "Song B", nil, 200, 128000, 44100, 2, "/m/b.mp3", 8, "mp3", "mp3", "", "", "h",
				0, 0, 0, 0, nil, nil, 1, "", now, now))
...
func TestDataTracksSearchFilter(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`FROM tracks WHERE library_id = ANY($1)`)).
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "library_id", "title", "cover_image_id",
			...
			"file_path", "file_size", "file_format", "audio_codec", "mbid", "acoust_id", "hash",
			"lyrics_mask", "lyrics_offset", "heat", "play_count", "last_played_at", "metadata", "version", "version_label", "created_at", "updated_at"}).
			AddRow("t-001", "lib-001", "Match Song", nil, 200, 128000, 44100, 2, "/m/a.mp3", 8, "mp3", "mp3", "", "", "h",
				0, 0, 0, 0, nil, nil, 1, "", now, now).
			AddRow("t-002", "lib-001", "Unrelated", nil, 200, 128000, 44100, 2, "/m/b.mp3", 8, "mp3", "mp3", "", "", "h",
				0, 0, 0, 0, nil, nil, 1, "", now, now).
			AddRow("t-003", "lib-001", "Version 2 Track", nil, 200, 128000, 44100, 2, "/m/c.mp3", 8, "mp3", "mp3", "", "", "h",
