...
func (e *Engine) applyNetworkLyrics(ctx context.Context, libraryID string, track *domain.Track, enrichment *metadata.EnrichmentResult) bool {
	...
	if err := e.lyricsStore.Save(ctx, libraryID, track.ID, lyrics.PriorityNetwork, enrichment.Lyrics); err != nil {
		if ctx.Err() != nil {
			logger.Info("[scan] network lyrics save cancelled for %s: %v (original: %v)", track.FilePath, ctx.Err(), err)
		} else {
			logger.Error("[scan] network lyrics save error for %s: %v", track.FilePath, err)
		}
		return false
	}
...
func (e *Engine) sweepOrphanLyrics(ctx context.Context, libraryID string) {
	...
	if err != nil {
		logger.Error("[scan] orphan lyrics glob error %s: %v", libDir, err)
		return
	}
	if len(matches) == 0 {
		return
	...
	if err != nil {
		logger.Info("[scan] orphan lyrics load track ids: %v", err)
		return
	}
	for rows.Next() {
		var id string
		if err := rows.Scan(&id); err != nil {
			rows.Close()
			logger.Info("[scan] orphan lyrics scan track id: %v", err)
			return
		}
		existing[id] = true
	}
	...
	if err := rows.Err(); err != nil {
		logger.Info("[scan] orphan lyrics iterate track ids: %v", err)
		return
	}
	...
	for _, f := range matches {
		...
		if err := os.Remove(f); err != nil && !os.IsNotExist(err) {
			logger.Info("[scan] remove orphan lyrics %s: %v", f, err)
		}
	}
}
