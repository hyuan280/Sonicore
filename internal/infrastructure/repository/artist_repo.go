...
func scanArtist(scanner interface{ Scan(dest ...interface{}) error }) (*domain.Artist, error) {
	...
	err := scanner.Scan(&a.ID, &a.Name, &a.SortName,
		&a.MBID, &a.Biography, &coverID, &a.TrackCount,
...
func (r *ArtistRepo) BatchCreate(ctx context.Context, artists []domain.Artist) error {
	...
	stmt, err := tx.PrepareContext(ctx,
		`INSERT INTO artists (id, name, sort_name, mbid, biography, cover_image_id, created_at, updated_at)
	...
	for _, a := range artists {
		_, err = stmt.ExecContext(ctx, a.ID, a.Name, a.SortName,
			a.MBID, a.Biography, a.CoverImageID, a.CreatedAt, a.UpdatedAt)
		if err != nil {
			return err
		}
	}
...
func (r *ArtistRepo) FindByID(ctx context.Context, id string) (*domain.Artist, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, name, sort_name, mbid, biography, cover_image_id, 0 AS track_count, created_at, updated_at
...
func (r *ArtistRepo) FindByLibraryID(ctx context.Context, libraryID string) ([]domain.Artist, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT DISTINCT a.id, a.name, a.sort_name, a.mbid, a.biography, a.cover_image_id, 0 AS track_count, a.created_at, a.updated_at
		 FROM artists a
...
func (r *ArtistRepo) FindAccessible(ctx context.Context, userID string) ([]domain.Artist, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT DISTINCT a.id, a.name, a.sort_name, a.mbid, a.biography, a.cover_image_id,
...
func (r *ArtistRepo) Update(ctx context.Context, artist *domain.Artist) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE artists SET name=$1, sort_name=$2, mbid=$3, biography=$4,
		 cover_image_id=$5, updated_at=NOW()
		 WHERE id=$6`,
