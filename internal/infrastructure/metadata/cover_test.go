...
func TestSafeCoverURL(t *testing.T) {
	...
	assert.False(t, safeCoverURL("http://0.0.0.1/x"), "0.0.0.0/8 routed locally on Linux")
	assert.False(t, safeCoverURL("http://100.64.0.1/x"), "CGNAT")
	assert.False(t, safeCoverURL("http://192.0.0.1/x"), "IANA special-use 192.0.0.0/24")
	assert.False(t, safeCoverURL("http://192.0.2.10/x"), "TEST-NET-1")
	assert.False(t, safeCoverURL("http://198.18.0.1/x"), "benchmarking 198.18.0.0/15")
	assert.False(t, safeCoverURL("http://240.0.0.1/x"), "reserved 240.0.0.0/4")
	assert.False(t, safeCoverURL("http://[2001:db8::1]/x"), "IPv6 documentation block")
	assert.False(t, safeCoverURL("http://[2001::1]/x"), "Teredo 2001::/32")
	assert.False(t, safeCoverURL("http://[2002:7f00:1::]/x"), "6to4 2002::/16 (maps to 127.0.0.1)")
	assert.False(t, safeCoverURL("http://[64:ff9b::a00:1]/x"), "NAT64 64:ff9b::/96 (maps to 10.0.0.1)")
	assert.False(t, safeCoverURL("http://[64:ff9b:1::a00:1]/x"), "NAT64 local-use 64:ff9b:1::/48 (maps to 10.0.0.1)")
	assert.False(t, safeCoverURL("http://[::a00:1]/x"), "IPv4-compatible ::/96 (maps to 10.0.0.1)")
	assert.False(t, safeCoverURL("http://[fec0::1]/x"), "site-local fec0::/10")
	assert.False(t, safeCoverURL("http://192.88.99.1/x"), "6to4 relay anycast 192.88.99.0/24")
