...
export default function PlatformSwitcher({ platforms, platform, onChange }: PlatformSwitcherProps) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
        ...
        const canScroll = (e.deltaY > 0 && el.scrollLeft < maxLeft) || (e.deltaY < 0 && el.scrollLeft > 0)
        if (!canScroll) return
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    e.preventDefault()
    const btns = Array.from(listRef.current?.querySelectorAll<HTMLButtonElement>('[role="tab"]') || [])
    if (btns.length === 0) return
    const current = Math.max(0, btns.findIndex(b => b.getAttribute("aria-selected") === "true"))
  ...
  return (
    ...
    >
      {platforms.map(p => (
        <button key={p.name} type="button" role="tab" aria-selected={platform === p.name}
          ...
          className={`px-4 py-1.5 text-sm transition-colors cursor-pointer whitespace-nowrap shrink-0 clip-plat ${
            platform === p.name ? "bg-green-600/25 text-green-500" : "bg-zinc-800 text-zinc-400 hover:text-white"
          }`}>
          {p.label}
        </button>
      ))}
