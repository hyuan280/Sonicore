...
import {
  ...
  openDesktopLyrics,
  closeDesktopLyrics,
  subscribeDesktopLyrics,
} from "../lib/desktopLyrics";
import {
...
const qualityOptions = [
  {
    key: "standard",
    labelKey: "player.qualityStandardShort" as const,
    titleKey: "player.qualityStandardDesc" as const,
    descKey: "player.qualityStandard" as const,
  },
  {
    key: "high",
    labelKey: "player.qualityHighShort" as const,
    titleKey: "player.qualityHighDesc" as const,
    descKey: "player.qualityHigh" as const,
  },
  {
    key: "lossless",
    labelKey: "player.qualityLosslessShort" as const,
    titleKey: "player.qualityLosslessDesc" as const,
    descKey: "player.qualityLossless" as const,
  },
] as const;

function loadQuality(): string {
  try {
    return localStorage.getItem("playback_quality") || "standard";
  } catch {
    return "standard";
  }
}
...
export default function PlayerBar() {
  ...
  if (prevVolume.current === 0.8) {
    try {
      const v = localStorage.getItem("prevVolume");
      if (v) prevVolume.current = parseFloat(v);
    } catch {}
  }
      ...
      fetch("/api/health", { signal: pollCtrlRef.current.signal, cache: "no-store" })
        ...
        .then((data) => {
          ...
          if (data?.status === "ok") {
            ...
            const finalPos =
              cur.position > 0 && cur.position !== resumePos ? cur.position : resumePos;
  ...
  useEffect(() => {
    if (ps.track) {
      api.user
        .checkFavorites([ps.track.id])
        .then((d) => setFav(!!d.favorites?.[ps.track!.id]))
        .catch(() => {});
    }
  }, [ps.track?.id]);
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-zinc-900 border-t border-zinc-800 px-4 py-2 z-50 h-16">
        ...
        <div
          ref={progressRef}
          className="absolute top-2 left-0 right-0 h-1 bg-zinc-800 cursor-pointer group overflow-hidden z-10"
          onClick={seek}
        >
          ...
          {track &&
            mse.buffered.map((r, i) => (
              <div
                key={i}
                className="absolute inset-y-0 bg-green-600/40"
                style={{
              ...
              <>
                <div className="w-10 h-10 rounded bg-zinc-800 flex-shrink-0 flex items-center justify-center text-xs text-zinc-500 overflow-hidden">
                  {track.cover_image_id ? (
                    <img
                      src={coverImageUrl(track.cover_image_id, 64)}
                      alt={track.title}
                  ...
                  <Music
                    className={`w-5 h-5 text-zinc-600 ${track.cover_image_id ? "hidden" : ""}`}
                  />
                </div>
          ...
          <div className="flex-1 flex items-center justify-center relative">
            <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
              ...
              <button
                ...
                title={modeTitles[ps.mode]}
              >
                {ps.mode === "shuffle" ? (
                  <Shuffle
                    className={`w-5 h-5 ${ps.mode === "shuffle" ? "text-green-500 group-hover:text-white" : "text-zinc-500 group-hover:text-white"}`}
                  />
                ) : ps.mode === "one" ? (
                  <Repeat1 className={`w-5 h-5 text-green-500 group-hover:text-white`} />
                ) : (
                  <Repeat
                    className={`w-5 h-5 ${ps.mode === "normal" ? "text-zinc-500" : "text-green-500"} group-hover:text-white`}
                  />
                )}
              </button>
                      ...
                      <div className="absolute bottom-full right-0 mb-1 w-64 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl z-[60] py-1 max-h-72 overflow-y-auto">
                        ...
                        <div className="border-t border-zinc-700 pt-1">
                          <div className="w-full text-left px-3 py-1.5 text-sm flex items-center gap-2 text-green-500">
                            ...
                            <span className="truncate">
                              {track.version_label ||
                                track.suffix?.toUpperCase() + " · " + t("player.current")}
                            </span>
                          ...
                          {track
                            .versions!.filter((v) => v.id !== track.id)
                            .map((v) => (
                              <button
                                key={v.id}
          ...
          <div className="flex items-center gap-3 w-80 justify-end">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => {
                  if (ps.volume > 0) {
                    prevVolume.current = ps.volume;
                    localStorage.setItem("prevVolume", String(ps.volume));
                    ps.setVolume(0);
            ...
            >
              ...
              <span className="text-[9px] font-semibold leading-none ml-0.5">
                {ps.queue.length || ""}
              </span>
            </button>
          </div>
          ...
          <div className="absolute bottom-full right-0 w-96 max-h-80 bg-zinc-900 border border-zinc-800 rounded-t-xl shadow-xl flex flex-col">
            <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800">
              <span className="text-xs text-zinc-500">
                {t("player.queue")} ({ps.queue.length})
              </span>
              <button
                onClick={ps.clearQueue}
            ...
            <div className="overflow-y-auto max-h-64 p-1">
              {ps.queue.map((t, i) => {
                return (
                  ...
                  >
                    ...
                    {t.version_label && (
                      <span
                        className="text-xs text-zinc-500 shrink-0 max-w-24 truncate group-hover:hidden"
                    ...
                    {t.artists && t.artists.length > 0 && (
                      <span className="text-xs text-zinc-500 shrink-0 group-hover:hidden">
              ...
              {ps.queue.length === 0 && (
                <p className="text-xs text-zinc-600 text-center py-4">{t("player.empty")}</p>
              )}
            </div>
          </div>
        )}
        {showQuality && (
          <div
            ref={qualityPanelRef}
            className="fixed z-[60] w-36 bg-zinc-800 border border-zinc-700 shadow-xl overflow-hidden rounded-t-xl"
          ...
          >
            {qualityOptions.map((o) => (
              ...
              >
                {qIcons[o.key]} {t(o.labelKey)}{" "}
                <span className="text-zinc-600 ml-1">{t(o.titleKey)}</span>
              </button>
            ))}
          </div>
        )}
