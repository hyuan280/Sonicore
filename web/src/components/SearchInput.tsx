...
interface SearchResult {
  tracks: { id: string; title: string; artists?: any[]; albums?: any[]; duration: number; suffix?: string }[]
...
interface SearchInputProps {
  ...
  showArtists?: boolean
  minLen?: number
  onSelectTrack?: (track: { id: string; title: string; albums?: { id: string; title?: string }[] }) => void
...
export default function SearchInput({
  ...
  const doSearch = useCallback(async (q: string) => {
    if (q.trim().length < minLen) { setResults(null); return }
    setLoading(true)
    try {
  ...
  const onChange = (val: string) => {
    ...
    if (val.trim().length < minLen) { setResults(null); setOpen(false); return }
