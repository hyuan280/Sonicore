    ...
    <div className="flex-1 min-w-0 bg-zinc-800 rounded-lg p-1.5 space-y-1">
      {artists.map((a, i) => (
        <div key={i} className="flex items-center gap-1 text-sm w-full">
          ...
          <button onClick={() => removeArtist(i)} className="p-0.5 text-zinc-500 hover:text-red-400 cursor-pointer shrink-0">
            <X className="w-3 h-3" />
          </button>
        </div>
      ))}
        ...
        <div className="space-y-1 border border-zinc-700 rounded-lg p-2 w-full">
          <div className="flex gap-1">
            <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              ...
              placeholder="Search artist..." className="bg-zinc-800 rounded px-2 py-1 text-sm flex-1 min-w-0 focus:outline-none focus:ring-1 focus:ring-green-500" autoFocus />
            <button onClick={doSearch} disabled={searching} className="p-1 text-zinc-400 hover:text-white cursor-pointer">
          ...
          {searching && <div className="text-center"><Loader2 className="w-4 h-4 animate-spin inline text-zinc-400" /></div>}
          {results.map((r, i) => (
            <button key={i} onClick={() => selectArtist(r)}
