...
export default function TrackTable({
  ...
  const displayOffset = serverPaged ? (page - 1) * perPage : (totalPages > 1 ? (page - 1) * perPage : 0)
  ...
  const handlePerPageChange = (val: number) => {
    if (!serverPaged) { setInternalPerPage(val); setInternalPage(1) }
    onPerPageChange?.(val)
  }
  ...
  const handleBulkQueue = useCallback(() => {
    player.addToQueue(displayedTracks.filter(t => selected.has(t.id)).map(t => ({
      id: t.trackId || t.id,
      title: t.title,
      duration: t.duration,
      suffix: t.suffix || "mp3",
  ...
  const handleBulkPlaylist = useCallback(async (plId: string) => {
  ...
  const commitPage = useCallback((val: string) => {
      ...
      <div className="sticky top-0 z-10 bg-black pb-2 space-y-2 px-6 pt-6">
                ...
                <div className="relative">
                  ...
                  {perPageOpen && (
                    <div className="absolute top-full left-0 mt-1 bg-zinc-800 rounded-lg shadow-xl z-50 py-1"
                      ...
                      {[10, 20, 50].map(n => (
                        <button key={n} onClick={() => handlePerPageChange(n)}
                      ...
                      {totalPages > 1 && (
                        <div className="absolute left-0 top-full mt-1 bg-zinc-800 rounded-lg shadow-xl z-50 py-1 max-h-48 overflow-y-auto min-w-[3rem]">
                          {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                            <button key={n} onMouseDown={() => { handlePageChange(n); setPageEditing(false) }}
        ...
        <div className="flex items-center gap-1 text-xs text-zinc-500 px-4 py-2 border-b border-zinc-800">
          <div className="flex items-center gap-1 w-1/2 shrink-0">
            {multi ? (
              <label className="flex items-center justify-center cursor-pointer shrink-0 w-10"
                onClick={selectAll}>
                <input type="checkbox" checked={selected.size === displayedTracks.length && displayedTracks.length > 0}
          ...
          <div className="flex items-center gap-1 flex-1">
            ...
            {extraColumnHeader && (
              <span className="min-w-[80px] max-w-[140px] shrink-0 text-center hidden sm:block">{extraColumnHeader}</span>
            )}
          ...
          return (
            <div key={t.id}
              ...
              <div className="flex items-center gap-1 w-1/2 min-w-0 shrink-0">
                ...
                <div className="w-7 shrink-0 justify-end inline-flex items-center" onClick={(e) => { e.stopPropagation(); onPlay(i); }}>
                ...
                <span className={`flex-1 min-w-[200px] text-sm truncate ml-3 cursor-pointer ${isCurrent ? "text-green-500" : ""}`}
                  onClick={() => onPlay(i)}>{t.title}</span>
              </div>
              <div className="flex items-center gap-1 flex-1 min-w-0">
                <span className="w-20 shrink-0 flex items-center justify-end gap-0.5">
                  <AddQueueBtn
                    track={{ id: tId, title: t.title, duration: t.duration, suffix: t.suffix, cover_image_id: t.cover_image_id, artists: t.artists, albums: t.albums }}
                    versions={t.versions}
                  />
                  <AddBtn trackId={tId} />
                  <FavBtn trackId={tId} initiallyFav={favoriteIds.has(tId)}
                ...
                {showAlbum && (
                  <span className="min-w-[120px] max-w-[280px] shrink-0 text-center hidden sm:block">
                    {t.albums?.[0]?.id ? (
                      <Link to={`/albums/${t.albums[0].id}`} className="text-sm text-zinc-500 truncate hover:text-white transition-colors" onClick={e => e.stopPropagation()}>{t.albums[0].title || ""}</Link>
                    ) : (
                      <span className="text-sm text-zinc-500 truncate">{t.albums?.[0]?.title || ""}</span>
                    )}
                  </span>
                )}
                {showDuration && (
