...
interface DirData {
  current: string; parent: string; dirs: DirEntry[]; has_parent: boolean
}
...
export default function DirectoryPicker({ open, initialPath, onClose, onSelect }: Props) {
  ...
  useEffect(() => {
    if (!open) return
    setInput(""); setError(""); setData(null)
    load(initialPath || "")
  }, [open])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-zinc-900 border border-zinc-700 rounded-xl w-full max-w-lg mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="h-64 overflow-y-auto p-2 space-y-0.5 relative">
          ...
          {data && (
            <>
              <button onClick={() => load(parentOf(data.current || data.parent))}
              ...
              {dirs.length === 0 && !loading ? (
                ...
                dirs.map(d => (
                  <div key={d.path}
                    ...
                    <button onClick={e => { e.stopPropagation(); setInput(d.path); load(d.path + "/") }}
        ...
        <div className="p-3 border-t border-zinc-800 flex justify-end gap-2">
          <button onClick={onClose}
            className="px-4 py-2 text-sm text-zinc-400 hover:text-white cursor-pointer">Cancel</button>
          <button onClick={() => {
            const v = input.endsWith("/") ? input : input + "/"
            if (input) { onSelect(v); onClose() }
