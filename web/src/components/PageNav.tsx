...
export default function PageNav({ page, totalPages, total, perPage, onPage, onPerPage }: PageNavProps) {
    ...
    <div className="flex items-center justify-end gap-2">
      ...
      <div className="flex items-center bg-zinc-800 rounded-lg">
        <div className="relative">
          <button type="button" aria-haspopup="menu" aria-expanded={perPageOpen}
          ...
          {perPageOpen && (
            <div className="absolute top-full left-0 mt-1 bg-zinc-800 rounded-lg shadow-xl z-50 py-1"
              ...
              {[10, 20, 30, 50].map(n => (
                <button type="button" key={n} onClick={() => { onPerPage(n); onPage(1) }}
        ...
        <span className="text-sm text-zinc-400 px-3 py-2">{page} / {shownTotalPages}</span>
        <span className="w-px h-4 bg-zinc-700" />
        <button type="button" disabled={page <= 1} onClick={() => onPage(page - 1)} aria-label={t("trackTable.prev")}
        ...
        <span className="w-px h-4 bg-zinc-700" />
        <button type="button" disabled={page >= totalPages} onClick={() => onPage(page + 1)} aria-label={t("trackTable.next")}
