...
export function useMseAudio(onFatal: () => void) {
  ...
  const appendBytes = useCallback((bytes: ArrayBuffer): Promise<void> => {
    ...
    if (sb.updating) {
      return new Promise<void>((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
          ...
          rej("sourcebuffer wait timed out");
        }, 10000);
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        rej("sourcebuffer append timed out");
      }, 10000);
      sb.addEventListener("updateend", onEnd);
      sb.addEventListener("error", onErr);
      try {
  ...
  const clearBuffer = useCallback((): Promise<void> => {
    ...
    if (sb.updating) {
      ...
      const retry = (): Promise<void> => {
        ...
        const chained2 = p.then(() => {
          if (clearWaitRef.current === chained2) clearWaitRef.current = null;
        });
    ...
    const chained = p.then(() => {
      if (clearWaitRef.current === chained) clearWaitRef.current = null;
    });
    clearWaitRef.current = chained;
    return chained;
  }, []);
  ...
  const fetchChunk = useCallback(
    async (chunk: number, startOverride?: number) => {
  ...
  const start = useCallback(
    (track: MseTrack, quality: string, resumeAt = 0, shouldPlay = false) => {
      const el = audioRef.current;
      if (!el) return;
      versionRef.current++;
      const ver = versionRef.current;
      ...
      ms.addEventListener("sourceopen", () => {
        ...
        if (ms.sourceBuffers.length === 0) {
          try {
            ms.addSourceBuffer(codecFor(quality));
          } catch {
            return;
          }
        }
