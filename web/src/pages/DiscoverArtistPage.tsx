...
export default function DiscoverArtistPage() {
  ...
  useEffect(() => {
    ...
    setArtist(null)
    setArtistError(null)
    api.platform.artist(platform, artistId).then(d => {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setTracksError(null)
    api.platform.artistTracks(platform, artistId, page, perPage).then(d => {
      if (cancelled) return
      setTracks(d.tracks || [])
      setTotal(d.total || 0)
    }).catch(err => {
          ...
          <div className="flex gap-6">
            ...
            <div className="flex flex-col justify-end min-w-0">
              ...
              <p className="text-sm text-zinc-500 mt-1">
                {artist.album_count > 0 ? `${t("album.totalAlbums", { count: artist.album_count })} · ` : ""}
                {artist.track_count > 0 ? `${t("artist.tracks", { count: artist.track_count })}` : ""}
              </p>
              {brief && (
                <div className="mt-2 max-w-2xl">
                  <p className={`text-sm text-zinc-500 whitespace-pre-line ${descOpen ? "" : "line-clamp-2"}`}>{brief}</p>
