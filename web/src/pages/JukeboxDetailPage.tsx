  ...
  return (
    <div className="pb-24">
      <div className="sticky top-0 z-10 bg-black pb-2 space-y-6 px-6 pt-6">
        ...
        {/* Now Playing */}
        <div
          className="flex items-center gap-4 p-4 border border-zinc-800 rounded-xl bg-zinc-900/50 cursor-pointer"
            ...
            {track && hasTracks ? (
              <>
                ...
                <div className="text-sm text-zinc-400 truncate">
                  {track.artist || t("jukebox.unknown")}
                </div>
        ...
        <div className="flex items-center gap-2 justify-center">
          ...
          <button onClick={handleMode} className="p-2 cursor-pointer" title={modeLabel}>
            <ModeIcon
              className={`w-5 h-5 ${playMode === "normal" ? "text-zinc-500" : "text-green-500"}`}
            />
          </button>
        ...
        {showSettings && id && (
          <PathMappingPanel jukeboxId={id} onClose={() => setShowSettings(false)} />
        )}
      </div>
      ...
      <div className="px-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-zinc-400">
            {t("jukebox.queue")} ({queue.length})
          </span>
          <div className="flex items-center gap-3">
            <button
              onClick={handlePushQueue}
              disabled={pushing || ps.queue.length === 0}
        ...
        <div className="space-y-0.5 border border-zinc-800 rounded-lg">
          ...
          {queue.map((tid: string, i: number) => {
            ...
            const displayTitle =
              isCurrent && track ? track.title : qt?.title || tid.substring(0, 12) + "...";
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (!id) return;
                  api.jukebox.play(id, tid).then((r) => r && setStatus(r));
                }}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(qt?.artists || displayAlbum) && (
                    <div className="text-xs text-zinc-500 truncate">
                      {qt?.artists ? <ArtistLink artists={qt.artists} /> : null}
                      {qt?.artists && displayAlbum ? " · " : ""}
                      {albumLink ? (
                        <Link
                          to={albumLink}
                ...
                {isCurrent && <Play className="w-3 h-3 text-green-500 flex-shrink-0" />}
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (!id) return;
                    api.jukebox.removeFromQueue(id, i).then((r) => r && setStatus(r));
                  }}
