...
import { Play, Clock, CheckSquare, Plus, ListPlus, Heart, Mic2 } from "lucide-react"
  ...
  return (
    <div className="p-6 space-y-6 pb-24">
      <div className="flex gap-6">
        <div className="w-48 h-48 rounded-full bg-zinc-800 flex-shrink-0 flex items-center justify-center">
      ...
      <div className="space-y-1">
        ...
        {tracks.map((t, i) => (
          <div key={t.id}
            className="flex items-center gap-2 px-4 py-1.5 rounded-lg hover:bg-zinc-800/50 cursor-pointer group">
            {multi ? (
            ...
            <div className="w-7 shrink-0 justify-end inline-flex items-center" onClick={() => player.setQueue(tracks, i)}>
              <span className="text-sm text-zinc-500 group-hover:hidden">{i + 1}</span>
              <Play className="w-3.5 h-3.5 text-green-500 hidden group-hover:inline" />
            </div>
            <span className="w-1/2 min-w-0 text-sm truncate ml-3 cursor-pointer"
