          ...
          <div className="flex gap-6 pb-2">
            <div className="w-48 h-48 rounded-full bg-zinc-800 flex-shrink-0 flex items-center justify-center overflow-hidden">
              {artist.cover_image_id ? (
                <img
                  src={coverImageUrl(artist.cover_image_id)}
                  alt={artist.name}
              ...
              <Mic2
                className={`w-12 h-12 text-zinc-500 ${artist.cover_image_id ? "hidden" : ""}`}
              />
            </div>
            <div className="flex flex-col justify-end">
              ...
              <p className="text-sm text-zinc-500 mt-1">
                ...
                {artist.roles ? " · " : ""}
                {allPlayerTracks.length} tracks
              </p>
              <button
                className="mt-4 w-fit px-4 py-2 rounded-lg bg-green-600 text-white text-sm font-medium hover:bg-green-500 cursor-pointer flex items-center gap-2"
                onClick={() => {
                  if (allPlayerTracks.length > 0) player.setQueue(allPlayerTracks, 0);
                }}
              >
