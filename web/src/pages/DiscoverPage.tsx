...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    api.platform.charts(platform).then(d => {
      ...
      if (!cancelled) { setCharts([]); setChartError(translateApiError(t, err)) }
    }).finally(() => {
            ...
            <Card className="hover:bg-zinc-800/50 transition-colors h-full p-0 overflow-hidden">
              ...
              <div className="p-3">
                ...
                <div className="flex items-center gap-2 text-xs text-zinc-500 mt-1">
                  ...
                  {c.track_count > 0 && <span>· {t("album.tracks", { count: c.track_count })}</span>}
                </div>
              </div>
            </Card>
          </Link>
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center space-y-3">
          ...
          <button onClick={() => { setListError(null); setRetryKey(k => k + 1) }}
            className="px-4 py-2 rounded-lg bg-zinc-800 text-sm text-zinc-300 hover:bg-zinc-700 cursor-pointer">
            {t("common.retry")}
          </button>
        </div>
  ...
  return (
    <>
      ...
      <div className="px-6 pb-24 space-y-6">
        {renderChartBody()}
        {platform && !loading && !chartError && charts.length === 0 && <p className="text-zinc-500 text-center py-12">{t("discover.noCharts")}</p>}
      </div>
    </>
  )
}
