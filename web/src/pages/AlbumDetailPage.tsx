          ...
          <div className="flex gap-6 pb-2">
            <div className="w-48 h-48 rounded-xl bg-zinc-800 flex-shrink-0 flex items-center justify-center overflow-hidden">
              {album.cover_image_id ? (
                <img
                  src={coverImageUrl(album.cover_image_id, 256)}
                  alt={album.title}
              ...
              <Disc3
                className={`w-12 h-12 text-zinc-600 ${album.cover_image_id ? "hidden" : ""}`}
              />
            </div>
            <div className="flex flex-col justify-end">
              ...
              <p className="text-sm text-zinc-500 mt-1">
                {album.year || ""}
                {album.country ? ` · ${album.country}` : ""} · {tracks.length} tracks
              </p>
              <Button
                className="mt-4 w-fit"
                onClick={() => {
                  if (tracks.length > 0) player.setQueue(makePlayerTracks(), 0);
                }}
              >
