...
export default function AlbumsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/data/albums?${params}`, {
  ...
  return (
    <>
      ...
      <div className="px-6 pb-24 space-y-6">
              ...
              <Link key={a.id} to={`/albums/${a.id}`} className="block">
                <Card className="hover:bg-zinc-800/50 transition-colors h-full p-0 overflow-hidden">
                  <div className="aspect-square flex items-center justify-center overflow-hidden bg-zinc-800">
                    ...
                    <Disc3
                      className={`w-8 h-8 text-zinc-600 ${a.cover_image_id ? "hidden" : ""}`}
                    />
                  </div>
                  <div className="p-3">
                    ...
                    <div className="flex items-center gap-2 text-xs text-zinc-500 mt-1">
                      ...
                      {a.song_count > 0 && (
                        <span>· {t("album.tracks", { count: a.song_count })}</span>
                      )}
                    </div>
                  </div>
                </Card>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {a.artistId ? (
                    <Link
                      to={`/artists/${a.artistId}`}
                      className="text-xs text-zinc-500 truncate hover:text-white transition-colors block"
                      onClick={(e) => e.stopPropagation()}
                    >
                      {a.artist || ""}
                    </Link>
        ...
        {albums.length === 0 && (
          <p className="text-zinc-500 text-center py-12">{t("trackTable.noAlbumsFound")}</p>
        )}
      </div>
    </>
  );
