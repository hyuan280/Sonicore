...
describe("player store", () => {
  ...
  describe("togglePlay", () => {
    ...
    it("starts shuffle head when in shuffle mode", () => {
      usePlayer.setState({ mode: "shuffle", shuffleOrder: [1, 0], shuffleIdx: 0, queue: [track("a"), track("b")] })
      usePlayer.getState().togglePlay()
  ...
  describe("cycleMode", () => {
    it("cycles normal → all → one → shuffle → normal", () => {
      usePlayer.getState().setQueue([track("a"), track("b")])
      const s = () => usePlayer.getState()
      s().cycleMode(); expect(s().mode).toBe("all")
      s().cycleMode(); expect(s().mode).toBe("one")
      s().cycleMode(); expect(s().mode).toBe("shuffle")
      s().cycleMode(); expect(s().mode).toBe("normal")
    })
  ...
  describe("addToQueue", () => {
    ...
    it("regenerates a valid shuffle order in shuffle mode", () => {
      usePlayer.setState({ mode: "shuffle", shuffleOrder: [0], shuffleIdx: 0, queue: [track("a")] })
  ...
  describe("removeFromQueue", () => {
    ...
    it("rewrites shuffle order after removal", () => {
      usePlayer.setState({ mode: "shuffle", shuffleOrder: [1, 2, 0], shuffleIdx: 0, queue: [track("a"), track("b"), track("c")], queueIdx: 1, track: track("b") })
      usePlayer.getState().removeFromQueue(0)
  ...
  describe("lyrics", () => {
    it("fetchLyrics stores result", async () => {
      mockedData.lyrics.mockResolvedValue({ lyrics: "[00:00.00]hi", format: "lrc", lyrics_offset: 0.5 })
