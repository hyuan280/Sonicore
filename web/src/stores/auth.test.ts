...
const AUTH_RESPONSE = {
  token: "tok", refresh_token: "rt", session_token: "st",
...
describe("useAuth", () => {
  ...
  it("login stores tokens and user", async () => {
    ...
    expect(s.user).toEqual({ id: "u1", username: "alice", email: "", role: "admin", created_at: "" })
  })
  ...
  it("loadUser sets user and role on success", async () => {
    mocked.me.mockResolvedValue({ id: "u1", username: "alice", email: "a@x.io", role: "user", created_at: "t" })
